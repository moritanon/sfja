...
(** マップ(または辞書)型は、ありふれたデータ構造です。一般的なソフトウェアを構築する場合や、特定のプログラミング言語理論の両方において。
以降の章でもたくさん使われることになるでしょう。それとこれまでの章で見た概念、高階関数からデータ構造の構築、
([Basics]と[Poly]の)、([IndProp]の)反映を使って無駄を省いた証明など、のよい事例になるでしょう。*)
...
(** 始める前に、一つ余談を。
    これまで見てきた章とは違い、一つ前の章を(推移的に全ての前の章も[Require Import]しません。
    そのかわりに、この章より後では、Coqの標準ライブラリから直接、我々が必要とする定義や定理をインポートします。
    あまり違いがないことに気がついたかもしれません。我々自身の定義や定理の名前を注意深く出来るだけ標準ライブラリと同じになるようにつけて来たからです。*)
...
(** 標準ライブラリのドキュメントは
    http://coq.inria.fr/library/.  にあります。

    [SearchAbout]コマンドは、特定の型のオブジェクトを含む定理を見つけることの出来る、良い方法です。*)
...
(** まず、我々のmapのindexつけの為にキーとなる型が必要になります。この目的のために、[List]の章から型[id]を再び使うことにします。この章の中で完結させるために、ここでもう一度定義を行って、同値性の比較関数や基本的な属性も一緒に定義します。*)
...
(** 次の有用な属性[beq_id]は数についての補題と似ています:*)
...
(** この章での主な仕事は、[Lists]の章で見たものと似たように振る舞う部分マップを定義し、その振舞いに付随する補題を追加することです。

今回、Key-Valueのペアのリストではなく、_関数_をマップを構成するために使用する予定なのですが。
この表現の利点は、マップのもっと外延的な、同じ方法でクエリされた二つのマップが文字通り同じものを
返す(同じ関数)であり、むしろ、"等価な"データ構造であるという、捉えかたが出来ることです。 これはすなわち、マップを使った証明を簡単にしてくれます。

二つのステップで部分マップを構築します。最初に、_全域マップ_(total map)を定義します。
マップに存在しないキーに対して、デフォルトの値を返すものです。*)
...
(*  Intuitively, a total map over an element type [A] _is_ just a
    ...
    to any id. *)
(** 直感的に、型Aを要素とする全域マップは、[id]を検索して[A]を返す唯の関数です。

[t_empty]関数は、デフォルトの要素を与えると、空の全域マップを生成します。このマップは、どんなidに対しても同じデフォルトの
要素を返します。*)

Definition t_empty {A:Type} (v : A) : total_map A :=
  (fun _ => v).

(** More interesting is the [t_update] function, which (as before) takes
    a map [m], a key [x], and a value [v] and returns a new map that
    takes [x] to [v] and takes every other key to whatever [m] does. *)
(** もうちょっと興味深い関数は、[t_update]です。(上と同じく)マップ[m]とキー[x]と値[v]を引数に取って
[x]を適用したら、[v]を返す、そして、他全部のキーでは[m]と同じ新しいマップを返します。*)
...
(** This definition is a nice example of higher-order programming.
    ...
    like this: *)
(** この定義は、高階関数のよい例です。[t_update]関数は、_関数_[m]を引数にとり、
望んだように動作をする新しい関数[fun x' => ... ]を返します。

例えば、[id]をとって[bool]に変換するマップが作れます。そのマップでは、[Id 3]は[true]にマップされていて
他の全てのキーでは、[false]にマップされています。このように:*)
...
(** This completes the definition of total maps.  Note that we don't
    ...
    application! *)
(** これで、全域マップの定義は終りです。[find]操作を定義する必要がないことに気をつけてください。
なぜなら単に関数として適用すればよいだけだからです! *)
...
(** 後の章で、マップを使うために、マップの振舞に関する、いくつかの本質的な事実が必要になります。
...
(*  First, if we update a map [m] at a key [x] with a new value [v]
    ...
    get back [v]: *)
(** まず、マップ[m]をキー[x]の値[v]で更新し、[x]を[update]の結果得られたマップから検索すると、[v]が得られます。*)
...
(** **** 練習問題: ★★, optional (t_update_neq)  *)
(*  On the other hand, if we update a map [m] at a key [x1] and then
    look up a _different_ key [x2] in the resulting map, we get the
    same result that [m] would have given: *)
(** 一方、マップ[m]をキー[x1]で更新したマップと元もマップで[x1]と異なるキー[x2]でキーを検索した結果が同じになること *)
...
(** **** 練習問題: ★★★, recommended (t_update_permute)  *)
