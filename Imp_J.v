...
[simpl]が常に成功するため、無限ループします。) Coqは自身の言語については、終了を保証していますが、Coqのタクティック言語については、そうではないのです。*)
(* ####################################################### *)
(** *** The [try] Tactical *)
...
(** [T]がタクティックのとき、タクティック [try T] は[T]と同様ですが、[T]が失敗するとき[try T] は(失敗せずに)何もしない点が違います。 *)
...
(** [try]タクティックを手動の証明で使用することは少し馬鹿げていると思われるかもしれまんが、こみいった自動化された証明において、[;]と同時に使用することで非常に便利になります *)
(* ####################################################### *)
...
(** [;]が最も使用されるのは、[T;T']ように、二つのタクティックを引数にとるやりかたです。最初にタクティック[T]が実行され、次にタクティック[T']が[T]によって生成された、それぞれのサブゴールにおいて実行されます。*)
