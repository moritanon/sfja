...
(** [Imp_J.v]でのImp言語の開発は、具象構文の問題を完全に無視しています。
    つまり、プログラマが書くアスキー文字列をデータ型[aexp]、[bexp]、[com]
    で定義された抽象構文木にどうやって変換するか、という問題です。
    この章では、 Coqの関数プログラミング機能によって簡単な字句解析器と構文解析器(パーサ)を構築することで、
    この残っている問題を終わらせます。

    ここでの全ての詳細を理解することは重要ではありません(よって説明はかなり少なく、練習問題もありません)。
...
(* ネストされたoptionEの上のマッチ式をより簡単に書くための糖衣構文。*)
